...
class banco extends fs_controller{
    ...
    public function __construct() {
        parent::__construct(__CLASS__, 'Bancos', 'Definiciones');
    }
    
    protected function private_core() {
        //INSTANCIA EL MODELO 
         $this->instanciamodel = new banco_m();
      ...
      if( isset($_POST['vnombre'])){     
        ...
        $instancianuevamodel->idbanco = $_POST['vcodigo'];
        ...
        ///GUARDA LOS DATOS SI NO DA ERROR
         ...
         {
            $this->new_message("Banco ".$instancianuevamodel->nombre." guardado correctamente.");
         }
         else
            ...
            if( $instancianuevamodel )
            {
               ...
               {
                  $this->new_message("Banco ".$instancianuevamodel->nombre." eliminado correctamente.");
               }
               else
  ...
  private function buscar()
   {
     ...
     //is_numeric = Comprueba si una variable es un número o un string numérico
      ...
      $data = $this->db->select("SELECT COUNT(idbanco) as total".$sql.';');
      if($data)
      {
