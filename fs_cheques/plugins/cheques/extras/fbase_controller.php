...
class fbase_controller extends fs_controller
{
    ...
    protected function fbase_buscar_cliente($query)
    {
        ...
        foreach ($cli->search($query) as $cli) {
            $nombre = $cli->nombre;
            if ($cli->nombre != $cli->nombre) {
                $nombre .= ' (' . $cli->nombre . ')';
            }

            $json[] = array('value' => $cli->nombre, 'data' => $cli->idcliente, 'full' => $cli);
        }
    ...
    public function fbase_get_subtotales_documento($lineas, $due_totales = 1)
    {
        ...
        /// ahora redondeamos
        foreach ($subtotales as $key => $value) {
            $subtotales[$key]['netosindto'] = round($value['netosindto'], 0, PHP_ROUND_HALF_DOWN);
            $subtotales[$key]['neto'] = round($value['neto'], 0, PHP_ROUND_HALF_DOWN);
            $subtotales[$key]['iva'] = round($value['iva'], 0, PHP_ROUND_HALF_DOWN);
            $subtotales[$key]['recargo'] = round($value['recargo'], 0, PHP_ROUND_HALF_DOWN);
            $subtotales[$key]['irpf'] = round($value['irpf'], 0, PHP_ROUND_HALF_DOWN);
        }
        ...
        //round($subtotales, 0, PHP_ROUND_HALF_DOWN)
    }
    
    
    
      //AGREGADO GIOVANNI PARA LAS COMPARAS
    public function fbase_get_subtotales_documento_compra($lineas, $due_totales = 1)
    {
        $subtotales = [];
        $irpf = 0;
        ...
        foreach ($lineas as $linea) {
            ...
            $subtotales[$codimpuesto]['recargo'] += $pvpcondto * $linea->recargo / 100;
            $irpf += $pvpcondto * $linea->irpf / 100;
            $subtotales[$codimpuesto]['ivaporcentaje'] =  $linea->iva;
        }
        ...
        foreach ($subtotales as $key => $value) {
            ...
            $subtotales[$key]['ivaporcentaje'] = $value['ivaporcentaje'];
        }
  ...
  //PARA SALON 1811 NRO TARJETA  
  protected function fbase_facturar_albaran_cliente1811($albaranes, $fecha = '', $codpago = '', $nrotarjeta='')
    {
        $continuar = TRUE;
